...
export const SettingsManager: React.FC<SettingsManagerProps> = ({
  ...
  const isLabelCategory = activeCategory?.startsWith('label_') || activeCategory?.startsWith('printer_');
  ...
  const handleUpdateTemplate = async (template: LabelTemplate) => {
    try {
      const updatedSettings = { ...settings };
      const index = updatedSettings.label_templates.findIndex((t: LabelTemplate) => t.id === template.id);
      ...
      case 'label_templates':
        ...
        return (
          <div className="space-y-4">
            {settings?.label_templates?.map((template: LabelTemplate, index: number) => (
              <div
                key={template.id}
              ...
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-300 mt-1">
                    These fields will be included on all label templates for food safety compliance.
                  </p>
                </div>
              </div>
            </div>
            
            {settings?.required_label_fields?.map((field: string, index: number) => (
              <div
                key={index}
        ...
        return (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {Array.isArray(settings?.[activeCategory || '']) && 
              settings[activeCategory || ''].map((item: string, index: number) => (
                <div
                  key={`${item}-${index}`}
