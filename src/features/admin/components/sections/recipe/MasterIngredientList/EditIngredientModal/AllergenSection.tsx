...
export const AllergenSection: React.FC<AllergenSectionProps> = ({
  ...
  const handleAllergenToggle = (allergenKey: string) => {
    // Convert snake_case to camelCase for database column names
    const camelKey = allergenKey.replace(/_([a-z])/g, g => g[1].toUpperCase());
    const key = `allergen${camelKey.charAt(0).toUpperCase() + camelKey.slice(1)}` as keyof MasterIngredient;
    onChange({
      ...formData,
      [key]: !formData[key]
    });
  ...
  const lowPriorityAllergens = [
    ...
    { key: 'hot_pepper', active: formData.allergenHotPepper },
