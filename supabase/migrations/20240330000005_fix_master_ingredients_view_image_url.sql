...
select
  ...
  COALESCE(
    (SELECT (jsonb_array_elements(media)->>'url')::text 
     FROM jsonb_array_elements(r.media) 
     WHERE jsonb_array_elements(r.media)->>'is_primary' = 'true' 
     LIMIT 1),
    null
  ) as image_url,
...
alter publication supabase_realtime add table master_ingredients_with_categories;