...
SELECT 
  ...
  modifier.first_name || ' ' || modifier.last_name AS modified_by_name,
...
LEFT JOIN 
  organization_team_members creator ON r.created_by::uuid = creator.id AND creator.organization_id = r.organization_id
LEFT JOIN 
  organization_team_members modifier ON r.modified_by::uuid = modifier.id AND modifier.organization_id = r.organization_id;
