...
-- Recreate the view with all the needed columns
...
SELECT 
  ...
  COALESCE(creator.first_name || ' ' || creator.last_name, 'Unknown') AS created_by_name,
  COALESCE(modifier.first_name || ' ' || modifier.last_name, 'Unknown') AS modified_by_name,
  COALESCE(creator.email, '') AS created_by_email,
  COALESCE(modifier.email, '') AS modified_by_email
FROM 
  recipes r
LEFT JOIN 
...
LEFT JOIN 
  organization_team_members creator ON creator.id = r.created_by::uuid AND creator.organization_id = r.organization_id
LEFT JOIN 
  organization_team_members modifier ON modifier.id = r.modified_by::uuid AND modifier.organization_id = r.organization_id;
